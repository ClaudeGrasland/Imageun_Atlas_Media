...
In the case of ANR-DFG IMAGEUN project, we distinguish between two cases of "where" that can refer to "states" or "regions". For this reason we create a new function called octocubes 
...
saveRDS(hc_day,"octocubes/hc_mycorpus_covid_states_regions_day.RDS")
paste("Size of resulting file = ",round(file.size("octocubes/hc_mycorpus_covid_states_regions_day.RDS")/1000000,3), "Mo")
```

We can see that the resulting object is rather small (0.7 Mo) which will make easier the production of visualization based on the crossing of the different dimensions.

If we want to wok only at week level, the hypercube could be even smaller (0.46 Mo)
...
saveRDS(hc_week,"octocubes/hc_mycorpus_covid_states_regions_week.RDS")
paste("Size of resulting file = ",round(file.size("octocubes/hc_mycorpus_covid_states_regions_week.RDS")/1000000,3), "Mo")

```

...and even smaller at month level (0.30 Mo) ...
...
saveRDS(hc_month,"octocubes/hc_mycorpus_covid_states_regions_month.RDS")
paste("Size of resulting file = ",round(file.size("octocubes/hc_mycorpus_covid_states_regions_month.RDS")/1000000,3), "Mo")

```


... or at year level (0.14 Mo)
...
saveRDS(hc_year,"octocubes/hc_mycorpus_covid_states_regions_year.RDS")
paste("Size of resulting file = ",round(file.size("octocubes/hc_mycorpus_covid_states_regions_year.RDS")/1000000,3), "Mo")

```


If we anticipate that the figures will propose various choices of visualisation at different time scale (day, week, month, year), it can be interesting to store all the octocubes in one single object taking the form of a list with a total size of 1.6 Mo

```{r}
hc<-list("day"=hc_day,"week"=hc_week,"month"=hc_month, "year"=hc_year)
saveRDS(hc,"octocubes/hc_mycorpus_covid_states_regions.RDS")
paste("Size of resulting file = ",round(file.size("octocubes/hc_mycorpus_covid_states_regions.RDS")/1000000,3), "Mo")
```
